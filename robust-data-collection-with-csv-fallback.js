...
async function robustDataCollectionWithCSVFallback() {
  const isFinalCollection = process.env.COLLECTION_TIME === "final";
  const targetDate = getTargetDate();
  
  console.log(`🚀 Starting ${isFinalCollection ? 'FINAL' : 'MID-DAY'} data collection with CSV fallback...`);
  ...
  for (const store of stores) {
    ...
    try {
      // Try to get data with CSV fallback
      const salesAmount = await getStoreDataWithCSVFallback(store, targetDate);
...
async function getStoreDataWithCSVFallback(store, targetDate) {
  ...
  try {
    ...
    console.error(`Data extraction failed for ${store.name}:`, error.message);
    return 0;
  } finally {
    await browser.close();
  }
...
// Run the collection
robustDataCollectionWithCSVFallback().then(() => {
